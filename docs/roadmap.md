...
- **v1.3.13–1.3.22（2026-06-03）**：用户心智升级：议题教练、发起预检、可取消生成、继续审议建议、历史任务筛选/备注、证据矩阵 MD、结果跳转工作台、快捷键与演示提示。
- **v1.3.3–1.3.12（2026-06-03）**：UI 组件全量迁移（`src/ui/Button`）、官网一屏一 primary、导出 HTML 议事厅字体、克制玻璃拟态、发布说明整理。
- **v1.3.2（2026-06-02）**：议事厅视觉重构与五级按钮体系；侧栏分组与场景溢出菜单。
- **v1.3.0（2026-06-02）**：用户场景库（内置 + 自定义，导入导出）；审议任务时间线（跨多场会议）。
- **v1.2.6（2026-06-02）**：证据矩阵 HTML/Markdown 导出（发言 + 证据池对照表）；会议历史搜索；议题模板库（6 类专业议题）。
...
3. 结果可信度
   ...
   - **v1.2.6**：导出证据矩阵 HTML（发言轮次 + 证据池对照表）。
   - 待做：检索源接入。
